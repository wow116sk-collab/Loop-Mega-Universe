...
with inversion $t(\tau)=t_{ev}\big[1-(1-\tau/\tau_{\max})^{3/2}\big]$ and rate $d\tau/dt\propto 1/M \propto (1-t/t_{ev})^{-1/3}$ — **divergent (integrably) into the flash**. Three readings: (i) the 10¹⁰⁰-yr pre-bang era is **finite** in τ; (ii) the clock approaches the bang as a **crescendo**, not a countdown; (iii) with the inflation stall appended, flash → hot bang is **one thick tick** — in entropic time the seam is nearly a single instant, however many e-folds sit inside it.
