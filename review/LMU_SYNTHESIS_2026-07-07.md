...
**So the LMU-specific open items reduce to exactly two:** *(i)* the **"+3" spread** — the un-derived dilution exponent (the two measured anchors demand k≈4.93; the hand-wave 2+3=5 matches, the rigorous 2+1.5=3.5 misses) [**soft**]; and *(ii)* the **wiring** ω = the prior aeon's conformal factor = the inflaton [**Hypo**]. **de Sitter stability, swampland, P>0 are field-wide questions LMU shares, not closure-blockers of LMU** — and LMU is no more exposed on them than any inflationary cosmology (often less).
...
| the "+3" spread | **[soft]** | "the two anchors demand k≈4.93; the hand-wave 2+3=5 hits it, the rigorous 2+1.5=3.5 misses — deriving the spread is open" | "derived / rigorous" |
...
LMU is a **hybrid assembly** that is data-pinned as a retrodiction of our own aeon and compatible with the CMB via a standard α-attractor. It contains **no new equation**; its one testable number (r=0.0037) is Starobinsky's, inherited conditional on the wiring. Its LMU-specific open items reduce to **exactly two**: the **"+3" spread** [soft] (the two measured anchors demand k≈4.93; the hand-wave 2+3=5 hits it, the rigorous 2+1.5=3.5 misses — deriving the spread is open) and the **[Hypo] wiring** (ω = the prior aeon's conformal factor = the inflaton). The reset is **deterministic** — the survivor *will* evaporate (Hawking, certain), its flash triggers the roll of a field already on the α-attractor plateau — so **de Sitter stability, the swampland metastable-minimum, and P>0 are NOT LMU closure-blockers**: they are field-wide questions every inflationary cosmology shares, and the deterministic flash removes them in their LMU-relevant forms (LMU is if anything less exposed — its thawing DE is swampland-favoured). What remains is the endgame spine choice (tiny-de-Sitter, chosen here) still to be propagated into the main `.tex`. The machinery runs and the values thread; the framework is **not** yet internally reconciled with the main document (the `.tex` still carries the old "V_min=0 / Minkowski" and "B-primary nucleation" wordings, edit pending approval), and no proof of closure exists. That is the honest state — including the parts the first draft of this synthesis glossed, which the stress test caught.
