...
{\small Worldview and architecture by Pitarn; equations, integrations, and the proven/assumed audit by the editor pass; assembled under the R6 protocol (all numbers recomputed, none inherited). 2026.}\\[2pt]
...
The body of the framework (the main V3.4 document) argues in words that ordinary matter climbs a two-way ladder of black holes, L4$\to$L1, ranked by an object's \emph{position in the eating process} rather than by its mass. Module L turned that picture into \emph{equations of motion}. \textbf{This module does three things with those equations and then explains what the results mean:}
...
\label{eq:spin}
\end{equation}
Prograde accretion drives $a_\ast\!\to\!a_{\rm eq}\!=\!0.998$ (Thorne 1974); many misaligned mergers random-walk it down as $a_\ast\!\approx\!a_0/\sqrt N$, whose differential form fixes $C_{\rm mrg}=\tfrac12$ (\emph{not} free; Hughes \& Blandford 2003). This is the \emph{merger-phase} spin-down that protects the survivor at the gate (\S5); it is distinct from the terminal Page spin-down at the very end.
...
&\qquad v_m=A_{\rm kick}\,\eta^2\tfrac{1-q}{1+q},\quad v_s=H_{\rm kick}\,\eta^2(a_2-q\,a_1)\cos\theta,\notag\\
\text{\bf die:}\quad & \text{quench}\iff M_g\to0\ \Rightarrow\ \dot\Mbh^{\rm acc}\to0\ \ (\text{starvation; Bullock et al.\ 2000}),
\end{align}
with $A_{\rm kick}=1.2\times10^4$, $H_{\rm kick}=6.9\times10^3$ km\,s$^{-1}$ (Gonz\'alez 2007; Campanelli 2007); the non-spinning recoil peaks at $\sim$175 km\,s$^{-1}$, superkicks of 2000--5000 km\,s$^{-1}$ need high in-plane spin. \Fact
...
$a_{\rm eq}=0.998$ (Thorne) & $K_{\rm mrg}=0.45$~Gyr$^{-1}$ (merger norm.) & $f_{\rm Edd}(t)$ (Eddington ratio) \\
...
\caption{\textbf{(a)} The BH mass function from the environment-weighted dynamics: declining and observed-like, with the L2 end rare \emph{because deep hosts are rare} ($n_{\rm L3}/n_{\rm L2}\!\approx\!90$, set by the VDF, not tuned). \textbf{(b)} How environments map to masses: the dynamics produces $M$--$\sigma$ at low $\sigma$ (field) and rises above it with merger spin-down at high $\sigma$ (cluster), plus the over-massive isolated-relic branch.}
\end{figure}

\textbf{What it means.} The L2 abundance is now first-principles \emph{given the VDF}: the top of the ladder is rare because the deep environments that build it are rare, a number that comes from the dispersion function rather than from a tuned merger rate. This remains the standard $\mathrm{BHMF}=\mathrm{VDF}\times M$--$\sigma$ construction --- non-discriminating --- the framework's additions being the over-massive relic branch and the spin structure; the VDF parameters and the environment model are approximate, and the low-mass (dwarf-IMBH) end is over-produced (observationally uncertain anyway). \Hyp

% =====================================================================
\section{What this model is for --- and what it cannot do}
\textbf{What it is for.} (i) A \emph{translator}: it converts the verbal ladder into a quantitative forward map, so the picture can be checked for internal consistency rather than only asserted. (ii) A \emph{fingerprinting tool}: because spin records assembly history (\S5), the model says that a direct spin measurement reads off whether a hole was merger-built or accretion-built --- a use with a concrete observational target (\S10). (iii) A \emph{population predictor of shape}: it says the mass function must be top-sparse, which is why no L1 is seen.
...
Starvation quenching & Bullock et al.\ (2000) \\
...
Integrated to $a=20.6$: the oscillation envelope decays $0.34\to0.004$; $\rho_{\rm DE}\!:2.07\to2.6\times10^{-4}$; $\rho_m\to10^{-4}$; $H\!:1.0\to0.011$. Power laws $\rho\propto a^{-3.00}$, $H\propto a^{-1.50}$, $\langle w\rangle=0.003$ --- exact matter-like dilution carrying $A\to\approx0$, $H\to0$. \Fact
...
\emph{Honest caveat.} The hump is \textbf{broad and shallow} (a $\sim$5\% variation in $\ddot a/a$ across $a=0.85$--$1.2$), not a sharp peak; and ``we sit at the summit'' is a consequence of choosing $m$ to fit $w_0$, so the why-now coincidence is relocated into $m$, not removed.

The reason acceleration ends is that $w_{\rm tot}$ rises through $-1/3$ (equivalently $\ddot a=0$) as $A$ thaws. The CPL form must \emph{not} be extrapolated past $a=1$: with our $(w_0,w_a)$ it gives $w(a{=}5)=+0.08$, $w(a{=}10)=+1.24$ (unphysical), whereas the real field stays bounded in $[-1,+1]$.
...
\end{tabular}\end{center}
\textbf{What is invariant} (output, not dialed): every potential lands in $w_0\in[-0.81,-0.99]$ on the $w>-1$ side, with $w_a<0$ (thawing) and $w\ge-1$ throughout. A clear relation emerges \emph{by itself} --- $w_0$ near $-1\Rightarrow$ small $|w_a|$; $w_0\approx-0.8\Rightarrow$ larger $|w_a|$ --- i.e.\ all potentials fall on one locus in the $(w_0,w_a)$ plane. This is the signature of a canonical scalar; it appears without fitting. \Fact
...
\item The $z_{\rm aeon}$ result is an impossibility argument, not a calculation; it rests on the conformal invariance of massless-radiation entropy, which is secure, and on 3.4's scoping of the boundary to the massless channel.
...
Crucially, by unitarity the evaporation leaves \emph{no} remnant: the entire $\Mbh c^2$ is returned to the exterior as radiation. So a zone's collapsed mass is \textbf{not} lost down a hole --- it re-emerges in $L0$.
...
\label{eq:dustera}
\end{equation}
(\Fact, FRW dust). Over one interlude ($\tau=2.1\times10^{100}$\,yr at the $10^{11}\,\msun$ band top, eq.~\eqref{eq:hawking}) the comoving causal reach is $\sim8\times10^{56}$\,m $\approx10^{26}$ island radii; mean densities dilute by $a^3\sim10^{180}$ (matter) while structure \emph{grows}:
\begin{equation}
...
\textbf{The ledger.} Summing the three channels, each zone deposits its rest energy $E_{\rm zone}\simeq\sum_i M_i c^2$ into $L0$ as radiation; the reservoir in turn feeds the assembly of new zones. At the level of the whole reservoir, with $\dot E^{\rm in}_{L0}$ the rate of deposit from dying zones and $\dot E^{\rm out}_{L0}$ the rate drawn into forming ones,
\begin{equation}
...
$g$ floor (only $n\,{\ge}\,10^9M_\odot$ holes, $10^{-5}\,$Mpc$^{-3}$) & $1.7\times10^{13}$ & BHMF only\\
$g$ fiducial (observable-universe budget) & $\mathbf{4.1\times10^{14}}$ & Egan--Lineweaver 2010\\
...
\own{Tolman 1934}
\end{equation}
with $g$ the \emph{measured} rate of Part~IV \S1 (floor $1.7\times10^{13}$; fiducial $4.1\times10^{14}$; post-ladder $\sim\!2\times10^{16}$, V3.7 recompute $7.4\times10^{16}$ --- reproduced here because this is the ledger that pays it).
\begin{center}
...
\textbf{Standing tension --- mandatory flag (travels with every reuse of the ladder):} the cluster spin-down expectation ($a_\star\sim0.55$) is in tension with the measured M87 spin $a_\star\gtrsim0.8$ (Drew et al.\ 2025) --- status \emph{leaning-against}, not falsified; the relic-side spin decoupling is unmeasurable in principle (Part~VI \S21). \textbf{V3.7 inversion:} the spin walk $a=a_0/\sqrt{1+N}$ (Hughes \& Blandford 2003) turns $a_\star\ge0.8$ into $N_{\rm misaligned}\le0.56$--$2.26$ (lower: pure walk from $a_0=0.998$; upper: walk allowing accretion re-spin, lift $\times1.45$) against the modeled $5.9$ --- tension factor $2.6$--$10.6$, i.e.\ an aligned-merger fraction $\ge62\%$ where the model assumed isotropy. Watch for new measurements (ngEHT/BHEX, $\sim$2030s).
...
$g\in(0,10]$ and the $\times2$/round kill bound & survivor-class truncation understated $g$ by $\sim$14 orders & measured $g$: floor $1.7\times10^{13}$, fiducial $4.1\times10^{14}$ (Part~IV \S1)\\
...
\noindent\textbf{Update, 2026-06 (restored from the archive's title page).} A later audit pass sharpened the status of the one distinctive claim (the relic/cluster spin decoupling). Two findings, folded into \S7 and the closing \S21 of this part: (i) the relic side is \emph{unmeasurable in principle} --- a relic's defining quiescence disables every spin probe (X-ray reflection, shadow illumination, merger-GW), so only a still-accreting relic is reachable; and (ii) the testable cluster side --- M87 --- now trends to high spin, which would not merely tension but \emph{collapse} the decoupling into the coherence parameter $f_{\rm coh}$. The claim's status moves from ``blocked-but-sharp'' to \textbf{leaning-against}, decided on the M87 side within $\sim$10 years (ngEHT/BHEX). \Auditor
...
M87* spin estimates & EHT Collaboration; Feng \& Wu (2017); Nokhrina et al.\ (2019); and others (model-dependent) \\
\bottomrule
\end{tabular}
\end{center}
...
median $\Delta a_\ast$ & $0.246$ & $0.45$\ \ (\textbf{differs}) \\
fiducial cluster $a_\ast$ & $0.751$ & $\sim0.55$\ \ (\textbf{differs}) \\
\bottomrule
\end{tabular}
\end{center}
...
\small
Abramowicz et al.\ (1988, slim discs); Andrei, Ijjas \& Steinhardt (2022, slow contraction); Bennett et al.\ (2024); Dotti et al.\ (2010); Fanidakis et al.\ (2011, chaotic-II); Gialamas et al.\ (2025, arXiv:2506.21542); GRAVITY/Abuter et al.\ (2024, BLR offsets); Inayoshi \& Ichikawa (2024, LRD density $\to$ high spin); King \& Pringle (2006, chaotic accretion); Lupi et al.\ (2024b, disc self-shadowing); Massonneau et al.\ (2023); Pacucci \& Narayan (2024); Rennehan et al.\ (2024, uncapped accretion); Reynolds (2024, coherent-accretion preference); Suh et al.\ (2025, LID-568); BlackHoleWeather (2025, chaotic-cold-accretion low spin). All used as owned results; the framework supplies only the readings. \Auditor
\normalsize
...
% ===================== PART VII =====================
% ===================== PART VII-DR =====================
\part{Stress audit with measured inputs, and Decision Record DR-Vmin}
\setcounter{section}{0}\setcounter{equation}{0}\setcounter{figure}{0}
...
\begin{figure}[h]\centering\includegraphics[width=0.95\textwidth]{l1_zoo_fates.png}
...
\item[\textbf{(J2)}] the sign of $V_{\min}$ --- the framework sits on the less-favoured fork; same observable as (B)'s data side (Part~VI \S18). \open\ \textbf{Mapped (V3.7):} all three branches are observationally identical today below $\epsilon_{\rm DESI}=3\times10^{-2}$ and share the DESI-Y5 falsifier; the discriminators are theoretical economy, not near-term data (Part~VII-DR).
...
\end{itemize}
With the M87 \emph{leaning-against} flag (Parts I, V, VI) attached wherever the ladder is reused.

% ---------------------------------------------------------------------
\section{Dead-end pointers (check before extending anything)}
\textbf{A1} --- DM origin, killed $\times5$ (Bullet Cluster $+$ timing $+$ 5:1); correct form: thermal relic re-minted each aeon. \dead\quad
\textbf{A2} --- DE/$\Lambda$ origin: rotation/vorticity (Saadeh et al.\ 2016 kills), Hawking-tail-as-DE, flash-as-A-source (short by 13--49 orders) --- all dead; in V3.4/3.5 dark energy is the A-field, no constant remains. \dead\quad
...
Fast/slow rotators ($\lambda_R$) & Emsellem et al.\ 2011; Cappellari & galaxy-spin contrast\\
Relic dynamical masses & Walsh et al.\ 2016; Cohn et al.\ 2021--2024 (ALMA); Krajnovi\'c et al.\ 2018 & the $n=6$ anchors\\
Relic sample; out-of-sample relics & Y{\i}ld{\i}r{\i}m et al.\ 2017; Newman et al.\ 2025 (MRG-M0138) & over-massiveness\\
Measured BH spins (X-ray reflection) & Reynolds 2013/2021; compilations to 2025 & spin-trend check\\
M87 spin measurement & Drew et al.\ 2025 (and EHT-era methods) & the leaning-against flag\\
...
Isotropic-singularity IVP shelf & Goode--Wainwright 1985; Newman 1993; Claudel--Newman 1998; Anguige--Tod 1999 I/II; Anguige 2000; Tod 2003, 2007; Lee--Nungesser--Tod 2020 & G2 anchors\\
$\Lambda>0$ smooth-$\mathscr I^+$ shelf & Friedrich 1986, 2017, 2023; L\"ubbe--Valiente Kroon 2013; Joudioux--Thaller--Valiente Kroon 2021; Had\v{z}i\'c--Speck 2015 & parent boundary closed\\
CCC two-sided matching (base point) & Kopi\'nski--Valiente Kroon 2022 & G1 solved sector\\
$\hat\Omega$ prescriptions (mutually inconsistent) & Newman 2014; Tod 2015; Nurowski 2021; Markwell--Stevens 2023 & conditional [G1-$\Omega$]\\
...
SMBH-binary GW background & NANOGrav: Agazie et al.\ 2023 & merger support; cleanliness bound\\
...
SLABs; dwarf occupation; topology & Carr--K\"uhnel--Visinelli 2020; Greene--Strader--Ho 2020; Planck 2015 XVIII & sweep (V3.12)\\\midrule
